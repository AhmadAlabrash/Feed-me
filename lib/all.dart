  ...
  Widget build(BuildContext context) {
    return BlocConsumer<Cubitnews, Statenews>(
        ...
        builder: (BuildContext context, Object? state) => ConditionalBuilder(
            condition: Cubitnews.getdata(context).all.length != 0,
            fallback: (context) => Center(
                    child: CircularProgressIndicator(
                  color: Cubitnews.getdata(context).isdark
                      ? Colors.white
                      : Colors.pink,
                )),
            builder: (context) => ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) => defultcadr(
                      Cubitnews.getdata(context).all[index], context),
                  separatorBuilder: (BuildContext context, int index) =>
                      defultdivider(context),
                  itemCount: Cubitnews.getdata(context).all.length,
                )));
  }
}
